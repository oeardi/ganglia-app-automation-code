...
public class AutomationEngineUtils {
    ...
    public static void uiAtuo(Map<String, Object> yamlToMap) {
        ...
        List stepsList = (List) yamlToMap.get(CONTAINT_steps);
        if (null == stepsList) {
            log.info("[调试信息] [uiAtuo] yaml 文件中的 [steps] 无内容，uiAtuo() 方法终止执行。脚本强制停止。");
            Reporter.log("【调试信息】 [uiAtuo] yaml 文件中的 [steps] 无内容，uiAtuo() 方法终止执行。脚本强制停止。");
            quit();
        }
        log.info("[调试信息] [uiAtuo] 输出 [yaml] 文件内容：");
        log.info("[调试信息] [uiAtuo] {}。", stepsList.toString());

        IPage pageObject = null;
        MobileElement mobileElement = null;

        Iterator iterator = stepsList.iterator();

        while (iterator.hasNext()) {
            log.info("[调试信息] [uiAtuo] [------------------------------------------------------------------------------------------]");
            Reporter.log("【调试信息】 [uiAtuo] [------------------------------------------------------------------------------------------]");

            Map<String, Object> stepsValueMap = (Map<String, Object>) iterator.next();
            for (Map.Entry<String, Object> entry1 : stepsValueMap.entrySet()) {
